...
public class EventManager : MonoBehaviour
{
    ...
    public void SetReferences(GameManager game, LocationUIManager locationUI, LocationActionManager actionManager, DialogueManager dialogue, RectTransform optionContainer)
    {
        gameManager = game;
    ...
    private void FindMissingReferences()
    {
        ...
        if (optionButtonContainer == null)
        {
            ...
            if (target != null) optionButtonContainer = target.GetComponent<RectTransform>();
        }
    }
    ...
    public void LoadEventData()
    {
        ...
        {
            if (eventData == null || string.IsNullOrEmpty(eventData.id)) continue;
    ...
    {
        if (cell == null || string.IsNullOrEmpty(cell.firstEnterEventId)) return;
        FindMissingReferences();
        if (gameManager == null) return;
        PlayerState playerState = gameManager.GetPlayerState();
        if (playerState == null) return;
        playerState.EnsureLists();
        if (playerState.HasVisitedCell(cell.id)) return;
    ...
    {
        if (eventById.Count == 0) LoadEventData();
        EventData eventData;
        if (string.IsNullOrEmpty(eventId) || !eventById.TryGetValue(eventId, out eventData))
        {
            if (locationUIManager != null) locationUIManager.ShowMessage("找不到事件数据：" + eventId);
            return;
        }
        ...
        ClearEventOnly();
        if (locationUIManager != null) locationUIManager.ShowEvent(eventData.title, eventData.text);
    ...
    private bool IsOptionVisible(EventOptionData option, PlayerState playerState)
    {
    ...
    private void ExecuteOption(EventOptionData option)
    {
        ...
        if (option.closeEvent) CloseEventAfterOptionMessage();
    }
    ...
    private void CloseEvent()
    {
        isEventOpen = false;
        ClearEventOnly();
        if (locationUIManager != null) locationUIManager.ShowMessage("事件已结束。");
        if (locationActionManager != null) locationActionManager.RefreshCurrentLocation();
    }
